#' Asymptotic Unimax test for testing equality means against umbrella-ordered alternatives in one-way ANOVA
#' @export
...
#' @details Testing of H_0:mu_1 = mu_2 = ... = mu_k vs H_1:mu_1 <=.....<= mu_(h-1)<= mu_h >= mu_(h+1)>=....>= mu_k (at least one strict inequality), where mu_i represents the population means of the i-th treatment. The input consists of two variables: sample_data and significance_level. The output consists of the critical value, the UniAMAX test statistic value, and the result, which indicates whether to reject or not reject the null hypothesis.
...
UniAMAX <- function(sample_data, significance_level, peak){
  ...
  quantile_value <- as.numeric(quantile(D_star_amax_values, probs = 1 - significance_level))

  # test statistic using your h-based split
  Aunimax <- max(c(
  ...
  result <- if (Aunimax > quantile_value) "Reject null hypothesis" else "Do not reject null hypothesis"

  return(paste("UniAMAX Critical value:", quantile_value, 
               "; UniAMAX Test statistic:", Aunimax, "; Result:", result))
}
